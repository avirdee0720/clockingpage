...
if( $emp->wendbonus == 1 ) { 
	...
	if($emp->paystru=="OLD")
	{
			...
			if($WeekendDaysToDate > 45) { 
				...
				//	if($AVWeekendDays >= 3.75) {
					 ...
					 $WeekendsDay45 = WeekendsDays452 ($clockinginno,$BonusYearStart,$dod,$CurrentBonusYearEnd);
       list ($WeekenDayTotal, $WeekendHoursTotal) = WeekendsThisMonth($clockinginno, $WeekendsDay45,$ddo);
	...
	if($WendBonus > 0.1 && $WeekendsDay45!=0) {
		echo "<TR><TD>$emp->pno - $WeekendsDay45</TD><TD>$emp->wendbonus</TD><TD>$emp->paystru </TD><TD>  $WeekenDayTotal </TD><TD> $WeekendHoursTotal </TD><TD> &pound; $WendBonus to add: $WeekendAdd </TD><TD>
