...
$title = "Average attendance all days incl days off (Mon-Sun)";
...
if ($db->Query($q)) {
	...
	//Regdays
        $q2 = "
		SELECT IFNULL(SUM(mon+tue+wed+thu+fri+sat+sun), 0) AS regd
        FROM regdays
        WHERE `no` = '$row->pno'
		  AND active = 'y'";
		...
		$q2 = "SELECT datediff('$ddo2','$dod2') as days";

        if (!$db1->Query($q2)) $db1->Kill();
		$row2=$db1->Row();
		if (!isset($row2->days)) $daysraw = 0;
		else $daysraw = $row2->days;
		$weeknumber = ($daysraw+1)/ 7.0;
        if ($weeknumber > 0)
...
<div id='reportexplanation'>
	...
	Hours per week = Amount of hours worked during the period / Number of weeks worked during the period.<br>
	Days per week = Number of days worked during the period / Number of weeks worked during the period.<br>
	Days per month = Number of days worked during the period / Number of months worked during the period.<br>
</div>
