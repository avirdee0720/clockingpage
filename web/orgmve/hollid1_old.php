...
$startedDAY=$rowfIRST->started;
	 ...
	 list ($urlop, $procent, $iledni, $LL) = Entitlement( $casual, $regday, $startedDAY, $startd);
...
if ($db->Query($q))
  {
	...
	if($row->hourgiven < 0.01) { $candelete="YES"; }  else  { $candelete="NO"; } 
     echo "
	 <tr>
         <td class='DataTD'><A HREF='holiddel.php?lp=$row->id&datad=$row->d1&candelete=$candelete&skrypt=$PHP_SELF?cln=$cln&yearAct=$YearAct&startd=$startd&endd=$endd'><IMG SRC='images/drop.png' BORDER='0' ALT='delete'></A></td>
