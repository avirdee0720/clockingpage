...
$db = new CMySQL;
$tytul="Casual's contact details";
...
if (!$db->Open()) $db->Kill();
$q = "SELECT `nombers`.`pno` , `nombers`.`surname` , `nombers`.`firstname` , `nombers`.`knownas` , `nombers`.`cat`, `emplcat`.catname, `staffdetails`.`homephone` , `staffdetails`.`mobilephone` FROM (`nombers` CROSS JOIN  `staffdetails` ON `nombers`.`pno` = `staffdetails`.`no`) INNER JOIN `emplcat` ON `nombers`.`cat`= `emplcat`.catozn WHERE `nombers`.`status` = 'OK'  AND  `cat`='c'";
$q=$q.$sortowanie;
