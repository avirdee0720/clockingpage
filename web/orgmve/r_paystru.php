...
$db = new CMySQL;
$tytul='Current staff pay structure';
...
<table border='0' cellpadding='3' cellspacing='1' CLASS='FormTABLE'>
  <tr>
    ...
    <td CLASS='ColumnTD' nowrap>
            ...
            <td CLASS='ColumnTD' nowrap>
                <A HREF='$PHP_SELF?sort=5&kier=$kier'>Pay str $kier_img[5]</a>
...
if(!isset($sort)) $sort=1;
        ...
        switch ($sort)
                ...
                case 4:
                 ...
                 break;
                case 5:
                 $sortowanie=" ORDER BY `nombers`.`paystru` $kier_sql";
                 break;
...
if (!$db->Open()) $db->Kill();
$q = "SELECT `nombers`.`ID`, `nombers`.`pno`, `nombers`.`firstname`, `nombers`.`surname`, `nombers`.`knownas`, `nombers`.`status`, `nombers`.`paystru`, `emplcat`.`catname` FROM `nombers` RIGHT JOIN `emplcat` ON `nombers`.`cat`=`emplcat`.`catozn` WHERE `status`='OK' AND `nombers`.`cat`!='ui'";
$q=$q.$sortowanie;
...
$row_count=0;
  ...
  if ($db->Query($q)) 
  {
	...
	echo "<tr>
		...
		<td CLASS='$row_color'>$row->paystru</td>
	</tr>";
  } 
} else {
echo " 
