 ...
 if(isset($cln)){  
  ...
  if (!$db->Query($sql)) { $db->Kill(); $db->Error(); exit; }
   while ($row=$db->Row())
    ...
    if ($name == "Payroll" || $name == "Janos Bodony") {
    echo "
...
echo "
	...
	<tr>      <td class='FieldCaptionTD'>speciality</td>
	  ...
	  <td class='DataTD'>$advances</td>
        ...
        <tr>
          <td class='FieldCaptionTD'>First clock in anywhere, then clock in/out only at the main office till 7PM, Mon-Fri</td>
...
$query2 = ("UPDATE `nombers` SET `trusted` = '$trust', `displ` = '$displ',`cat`='$category',`cattoname`='$cattoname',`advances`='$advances',`cloffice`='$cloffice',`cloffice_only`='$cloffice_only',`overtime_fl`='$overtime',`moretrday`='$moretrialday',`weekendrequired`='$weekendrequired',`rent`='$rent',`started`=STR_TO_DATE( '$started', '%d/%m/%Y') $attsql WHERE `pno` = '$cln' LIMIT 1 ;");
 // echo $query2;
