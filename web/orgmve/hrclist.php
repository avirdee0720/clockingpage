...
if (!$db->Open()) $db->Kill();
$q = "SELECT `nombers`.`pno`, `nombers`.`surname`, `nombers`.`firstname`, `nombers`.`knownas`, `nombers`.`cat`, DATE_FORMAT(`nombers`.`started`, \"%d/%m/%y\") as d1,`staffdetails`.`decision` ,DATE_FORMAT(`staffdetails`.`decisiondate`, \"%d/%m/%y\") as decisiondate,`emplcat`.catname    FROM (`nombers` CROSS JOIN  `staffdetails` ON `nombers`.`pno` = `staffdetails`.`no`) INNER JOIN `emplcat` ON `nombers`.`cat`= `emplcat`.catozn WHERE `nombers`.`status` = 'OK' AND  `nombers`.`cat` = 'c'";
$q=$q.$sortowanie;
...
$row_count=0;
 ...
 $decisiontxt = $decisionarray[$row->decision];
  ...
  <td CLASS='$row_color' nowrap=\"nowrap\"><div align=\"center\">$row->catname&nbsp;</div></td>
  <td CLASS='$row_color' nowrap=\"nowrap\"><div align=\"right\">$row->d1&nbsp;</div></td>
	<td CLASS='$row_color' nowrap=\"nowrap\"><div align=\"right\">$row->decisiondate&nbsp;</div></td>
	<td CLASS='$row_color' nowrap=\"nowrap\"><div align=\"center\">$decisiontxt&nbsp;</div></td>
		</tr>"; 
  } 
} else {
	$echo=  " 
...
<table border='0' cellpadding='2' cellspacing='1' CLASS='FormTABLE2' width=\"98%\">
  <tr>
   ...
   <td CLASS='ColumnTD2' nowrap=\"nowrap\"><div align=\"center\">
        ...
        <td CLASS='ColumnTD2' nowrap=\"nowrap\"><div align=\"center\">
         ...
         <td CLASS='ColumnTD2' nowrap=\"nowrap\" width=\"100\" ><div align=\"center\">
                <A HREF='$PHP_SELF?sort=7&kier=$kier'>Decision$kier_img[7]</a></div></td>
