...
//(!isset($_POST["leave_entitl_percent"])) ? $leave_entitl_percent = "" : $leave_entitl_percent = $_POST["leave_entitl_percent"];
...
(!isset($_POST["cloffice_only"])) ? $cloffice_only = 0 : $cloffice_only = $_POST["cloffice_only"];
(!isset($_POST["assessment"])) ? $assessment = 0 : $assessment = $_POST["assessment"];
(!isset($_POST["overtime"])) ? $overtime = 0 : $overtime = $_POST["overtime"];
...
if (isset($cln)) {
    ...
    SELECT n.pno,
        ...
        n.weekendrequired,
        n.`rent`,
        DATE_FORMAT(n.started, '%d/%m/%Y') as d1,
        DATE_FORMAT(n.dateforattendence, '%d/%m/%Y') as dateforattendence,
        n.daylyrate,
        n.`regdays`,
        n.displ,
        n.cloffice,
        ...
        n.assessment,
        n.trusted,
        n.paid_leave_agreed_to,
        n.paid_leave_estimate,
        sd.homephone,
        sd.mobilephone,
        sd.email AS email1,
        sd.jobtitle,
        ...
        sd.dooraccess,
        sd.managament_bonus,
        sd.punctuality_bonus,
        sd.security_bonus,
        sd.travel_card_bonus,
        sd.address1,
        sd.address2,
        sd.address3,
        sd.address4,
        ...
        sd.comment5,
        sd.comment6,
        DATE_FORMAT(sd.approvedon, '%d/%m/%Y') AS approvedon,
        sd.grade,
        sd.approved,
        sd.`old`,
        sd.speciality,
        sd.aproved,
        DATE_FORMAT(sd.decisiondate, '%d/%m/%Y') AS decisiondate
    FROM nombers n
    LEFT JOIN staffdetails sd ON n.pno = sd.`no`
    WHERE n.pno = $cln
    LIMIT 1
    ";
} else { 
...
$clockingNO = $row->pno;
    ...
    echo "
		...
		<a CLASS='DataLink' href='pay01.php?cln=$clockingNO'><IMG SRC='images/cons_report.gif' WIDTH='16' HEIGHT='16' BORDER='0' TITLE='PAYs'>PAY</a>
    	        <a CLASS='DataLink' href='hollid1.php?cln=$clockingNO&yearAct=$YearAct'><IMG SRC='images/hollidays.gif' WIDTH='16' HEIGHT='16' BORDER='0' TITLE='Holidays'>HOLIDAYS</a>";
    }
...
/*
	<a CLASS='DataLink' href='tt_usermodif.php?cln=$clockingNO'><IMG SRC='images/tt_timetable.png' WIDTH='16' HEIGHT='16' BORDER='0' TITLE='Modify TimeTable Informations'>TimeTable Informations</a>
...
if ($row->cloffice_only == 0)  {$cloffice_only="<INPUT TYPE='checkbox' NAME='cloffice_only'>";}
	else {$cloffice_only="<INPUT TYPE='checkbox' NAME='cloffice_only' checked>";}
if ($row->overtime_fl == 0)  {$overtime="<INPUT TYPE='checkbox' NAME='overtime'>";}
	else {$overtime="<INPUT TYPE='checkbox' NAME='overtime' checked>";}
if ($row->assessment == 0)  {$assessment="<INPUT TYPE='checkbox' NAME='assessment'>";}
	else {$assessment="<INPUT TYPE='checkbox' NAME='assessment' checked>";}
...
<td class='DataTD' colspan='4'><table border='0' cellpadding='3' cellspacing='1' class='FormTABLE'>
	...
	<tr>     <td class='FieldCaptionTD'>Category on Clocking Table</td>
	  <td class='DataTD'><input class='Input' size='30' maxlength='50' name='cattoname' value=\"$row->cattoname\"></td>
	</tr>
	";
	/*<tr>     <td class='FieldCaptionTD'>Leave entitlement %</td>
	  <td class='DataTD'><input class='Input' size='10' maxlength='2' name='leave_entitl_percent' value=\"$row->leave_entitl_percent\"></td>
	</tr>";
	*/
...
if ($row->paid_leave_agreed_to)
    $paid_leave_agreed_to_checked = "checked";
else
    $paid_leave_agreed_to_checked = "";
...
echo "
	...
	<tr>
      <td class='FieldCaptionTD'>Approved </td>
	  <td class='DataTD'>$approved</td>
 	</tr>
	<tr>     <td class='FieldCaptionTD'>Approved On</td>
	  <td class='DataTD'><input class='Input' size='12' maxlength='15' name='approvedon' value=\"$row->approvedon\"> (dd/mm/yyyy)</td>
	</tr>
	<tr>
      ...
      <input type='radio' name='trusted' value='1' ";
       ...
       if ($row->trusted == 1) echo "checked"; echo ">Appraisal complete<br>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<input type='radio' name='trusted' value='0' "; if ($row->trusted == 0) echo "checked";
    
    echo ">Appraisal incomplete</td>  
	</tr>
    ...
    <tr>      <td class='FieldCaptionTD'>Paid leave days agreed to</td>
	  <td class='DataTD'><input type='checkbox' name='paid_leave_agreed_to' value='1' $paid_leave_agreed_to_checked onclick='document.getElementById(\"worked_hours_estimate\").disabled = !this.checked;'></td>
    </tr>
    <tr>      <td class='FieldCaptionTD'>Estimation of worked hours for current leave year</td>
	  <td class='DataTD'><input type='text' name='worked_hours_estimate' id='worked_hours_estimate' size='20' maxlength='8' value='$row->paid_leave_estimate'"; if ($row->paid_leave_agreed_to != 1) echo "disabled"; echo "></td>
    </tr>
	<tr>      <td class='FieldCaptionTD'>speciality</td>
	  ...
	  <td class='DataTD'>$advances</td>
        ...
        <tr>
          <td class='FieldCaptionTD'>Clock in at the main office Mon-Fri</td>
          <td class='DataTD'>$cloffice</td>
        </tr>
        <tr>
          <td class='FieldCaptionTD'>First clock in enywhere, then lock in/out only at the main office till 7PM, Mon-Fri</td>
          <td class='DataTD'>$cloffice_only</td>
        </tr>
        <tr>
          <td class='FieldCaptionTD'>Clock in/out bef. 9.40</td>
          <td class='DataTD'>$overtime</td>
	</tr>
        <tr>
          <td class='FieldCaptionTD'>Assessment</td>
          <td class='DataTD'>$assessment</td>
	</tr>
	</tr>
</TABLE></td>
...
<BR>";
if ($name == "Payroll" || $name == "Egert Simson") {
echo "<table border='0' cellpadding='3' cellspacing='1' class='FormTABLE'>	<tr>
<tr>
...
if( $cloffice_only !== 0 ) $cloffice_only = 1;
...
if ($dateforattendence != "")
    $attsql = ",`dateforattendence` = STR_TO_DATE( '$dateforattendence', '%d/%m/%Y')";
else
    $attsql = ",`dateforattendence` = NULL";

if (array_key_exists('paid_leave_agreed_to', $_POST))
    $paid_leave_agreed_to = 1;
else
    $paid_leave_agreed_to = 0;
$worked_hours_estimate = $db->Fix(intval($_POST['worked_hours_estimate']));
...
$query = "UPDATE `staffdetails` SET `jobtitle` = '$jobtitle',`terminationtype` = '$terminationtype', " .
         "`address1` = '$address1',`address2` = '$address2',`address3` = '$address3',`address4` = '$address4', " .
		 "`postcode` = '$postcode', `decision` = $decision, `comment1` = '$comment1' , `comment2` = '$comment2' , " .
		 "`comment3` = '$comment3', `homephone` = '$homephone', `mobilephone` = '$mobilephone' ,`email` = '$email', " .
		 "`approved` = '$approved',";
if (($approved == 1) && ($approvedon != "")) {
		$query .= "`approvedon` = STR_TO_DATE('$approvedon','%d/%m/%Y'), ";
}
$query .= "`grade` = '$grade', `speciality` = '$speciality', `pr14` = '$_pr14' , `pr16` = '$_pr16', `pr20` = '$_pr20', `pr28` = '$_pr28', `pr30` = '$_pr30', `pr32` = '$_pr32', `pr34` = '$_pr34', `nhg56` = '$_nhg56', `nhg34` = '$_nhg34', `nhg36` = '$_nhg36', `nhg38` = '$_nhg38', `nhg40` = '$_nhg40', `nhg42` = '$_nhg42', `chs` = '$_chs', `bs` = '$_bs', `gcs` = '$_gcs', `247` = '$_d247' $decisionsql WHERE `NO` =$cln LIMIT 1 ;";
...
$query2 = "
    ...
    SET `trusted` = '$trust',
        ...
        `advances` = '$advances',
        `cloffice` = '$cloffice',
        `cloffice_only` = '$cloffice_only',
        `assessment` = '$assessment',
        `overtime_fl` = '$overtime',
        `moretrday` = '$moretrialday',
        `weekendrequired`='$weekendrequired',
        `rent`='$rent',
        `started` = STR_TO_DATE('$started', '%d/%m/%Y'),
        paid_leave_agreed_to = $paid_leave_agreed_to,
        paid_leave_estimate = $worked_hours_estimate
        $attsql
    WHERE `pno` = '$cln' LIMIT 1 ;
";
