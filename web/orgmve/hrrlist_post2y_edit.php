...
if (!$db->Open()) $db->Kill();
$q = "SELECT DISTINCT `nombers`.`pno`, `nombers`.`surname`, `nombers`.`firstname`, `nombers`.`knownas`, `nombers`.`cat`, `nombers`.`textperson`, `nombers`.`sign`, 
DATE_FORMAT( `nombers`.`started` , \"%d/%m/%y\" ) AS d1, `emplcat`.catname, `emplcat`.catname_staff, `staffdetails`.`decision`
FROM (`nombers` CROSS JOIN  `staffdetails` ON `nombers`.`pno` = `staffdetails`.`no`) 
INNER JOIN `emplcat` ON `nombers`.`cat`= `emplcat`.catozn WHERE `nombers`.`status` = 'OK' AND  `nombers`.`cat` <> 'c' 
AND  `nombers`.`pno` <> '5'
...
AND DATEDIFF( now( ) , started ) >= 365*2
...
$q = "SELECT id, value, signcode FROM `singlist`";
  ...
  <td CLASS='$row_color' nowrap=\"nowrap\"><b><div align=\"right\">$row->d1&nbsp;</div></b></td>
	<td CLASS='$row_color' nowrap=\"nowrap\" width='280'><b><div align=\"right\"><input class='Input' size='42' maxlength='120' name='textperson[]' value=\"$row->textperson\"></b></td>
