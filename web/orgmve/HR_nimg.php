...
if(isset($_POST['submitted'])) {
    $lp = $_POST['lp'];
    $data = addslashes(file_get_contents($_FILES['src']['tmp_name']));
    $opis = addslashes(nl2br($des));
    $src_name = addslashes($_FILES['src']['name']);
    $src_size = getimagesize($_FILES['src']['tmp_name']);
    $src_type = addslashes($src_size['mime']);
    $src_size = $src_size[3];
