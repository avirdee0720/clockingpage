...
$tytul='Punctuality bonus OLD Scale, count, store, and export (display)<BR><BR>';
...
echo "<TABLE><tr><td>CL no</td><td>TIME WORKED</td><td>WEEKEND TIME</td><td>TOTAL DAYS</td><td>WEEKEND DAYS</td><td>Punct %</td></td><td>Punct Bonus</td></tr>";
...
if($emp->paystru=="OLD")
	...
	$hourlyrate = $emp->daylyrate / 8.5;
	$grosspay = $hourstogether * $hourlyrate;
	$punctBonus = ($grosspay * 0.07) * $punctualpercent2;
	$punctBonus = number_format($punctBonus,2,'.',' ');
	}
	else { 
		//echo "$emp->pno error in pay structure or penalty<br>"; 
	$hourstogether =0;
	$hourlyrate = 0;
	$grosspay = 0;
	$punctBonus = 0;
...
if($punctBonus > 0.1) { echo "<tr><td>$emp->pno</td><td>$workedt</td><td>$weekendst</td><td>$alldayst</td><td>$wdt</td><td>$punctualpercent % </td><td>$punctBonus</td></tr>";

	if (!$InsertDB1->Open()) $InsertDB1->Kill();
	$delsql="DELETE FROM `paypunctuality` WHERE `no`='$emp->pno' AND `date1`='$ddo'";
	if (!$InsertDB1->Query($delsql)) $InsertDB1->Error();
	$InsertDB1->Free();

	if (!$InsertDB->Open()) $InsertDB->Kill();
	$inserttodb="INSERT INTO `paypunctuality` ( `id` , `no` , `monetarytotal` , `punctprc` , `hours` , `days` , `daysintime` , `type` , `sageno`, `date1` )VALUES (NULL , '$emp->pno', '$punctBonus','$punctualpercent', '$workedt',  '$alldaysINOUT', '$przedczasem', 'addition', '3', '$ddo')";
