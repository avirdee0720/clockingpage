...
{
echo "<CENTER>
<table WIDTH=95% bordercolorlight='#000000' border='border' style='border-collapse: collapse; background-color: white' >
  ...
  while($w=$baza->Row())
 {
    ...
    FROM hd_status RIGHT JOIN hd_zgloszenie ON hd_status.lp = hd_zgloszenie.stan WHERE hd_zgloszenie.stan <> 10 AND hd_zgloszenie.stan <> 0 ORDER BY hd_zgloszenie.stan ASC, hd_zgloszenie.data DESC";
 }
 ...
 while($wiersz=$baza->Row())
 {
   ...
   print("
      ...
      <td class='DataTD'>$wiersz->lokalizacja</td>
		<td class='DataTD'> $wiersz->stopis</TD>
...
{      
      $baza = new CMySQL;
	   if (!$baza->Open()) $baza->Kill();
       $da=$data.", ".$teraz;
       $anuluj = "UPDATE hd_zgloszenie SET stan='$st1', obsluzyl='$UID' ,dataobsl='$da' WHERE lp='$lp' LIMIT 1";
