...
if (!$db->Open()) $db->Kill();
$sql = "SELECT `nombers`.`pno`, `nombers`.`knownas`, `nombers`.`firstname`, `nombers`.`surname`, `nombers`.`status`, `nombers`.`regdays`, `nombers`.`started`, `nombers`.`left1` , `nombers`.`cat`, `nombers`.`from1`, `nombers`.`to1`,  `nombers`.`bonustype`, `emplcat`.`catname`, (`nombers`.`regdays` * 4) AS enti FROM `nombers`  JOIN `emplcat` ON `emplcat`.`catozn` = `nombers`.`cat`  WCHERE `pno`='$cln' ORDER BY `nombers`.`pno` ";
$q=$sql;
