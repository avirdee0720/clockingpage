...
echo "
<font class='FormHeaderFont'>Left in $month/$year</font>
<BR>
<table WIDTH=500 bordercolorlight='black' border='border' style='border-collapse: collapse; background-color: white' >";

if (!$db->Open()) $db->Kill();
$q = "SELECT `nombers`.`ID`, `nombers`.`pno`, `nombers`.`firstname`, `nombers`.`surname`, `nombers`.`knownas`, `nombers`.`status`, DATE_FORMAT(`nombers`.`left1`, \"%d/%m/%Y\") as d2, `emplcat`.`catname` FROM `nombers` LEFT JOIN `emplcat` ON `nombers`.`cat`=`emplcat`.`catozn` WHERE `status`='LEAVER' AND `nombers`.`left1` LIKE '$year-$month%'";
