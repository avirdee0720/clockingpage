...
$db = new CMySQL;
$db1 = new CMySQL;
$tytul='Staff start date Post 1 Year';
...
if (!$db->Open()) $db->Kill();
$q = "SELECT `n`.`ID`, `n`.`pno`, `n`.`firstname`, `n`.`surname`, `n`.`knownas`, `n`.`status`, `n`.`cat`, `n`.`started`, `details`.`homephone` , `details`.`mobilephone`, `cat`.`catname` FROM `nombers` n CROSS JOIN  `staffdetails` details ON `n`.`pno` = `details`.`no` LEFT JOIN  `emplcat` cat ON `n`.`cat` = `cat`.`catozn` WHERE `status` LIKE 'OK' "; 
$q=$q.$sortowanie;
...
if ($db->Query($q)) 
  {
	...
	if($HowManyDaysFromStart <> "NONE" && $HowManyDaysFromStart > 365)
		{
			$row_count++;
