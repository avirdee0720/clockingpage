...
$tytul='Punctuality in period of time<BR><BR>';
...
echo "<TABLE><tr><td>CL no</td><td>TIME WORKED</td><td>WEEKEND TIME</td><td>TOTAL DAYS</td><td>WEEKEND DAYS</td><td>Punct %</td></td><td> &nbsp;</td></tr>";
...
$prac1 =("SELECT `ID`, `pno`, `firstname`, `surname`, `knownas`, `status`, `puncbonus`, `daylyrate`, `paystru`, `left1` FROM `nombers` Where`nombers`.`status`= 'OK' ORDER BY `pno` ASC");
...
while ($emp=$PuncBonusAllempDBl->Row())
{
	...
	if (!$PuncBonusDB3->Open()) $PuncBonusDB3->Kill();
		...
		if(isset($soboty->wd) && isset($niedziele->wd)) $wdt=$soboty->wd + $niedziele->wd; 
		elseif (isset($soboty->wd) && !isset($niedziele->wd)) $wdt=$soboty->wd;
		elseif (!isset($soboty->wd) && isset($niedziele->wd)) $wdt=$niedziele->wd;
		else $wdt="";
...
echo "<tr><td>$emp->pno</td><td>$workedt</td><td>$weekendst</td><td>$alldayst</td><td>$wdt</td><td>$punctualpercent % </td><td>&nbsp;</td></tr>";
