...
<font class='FormHeaderFont'>Less than 3 <BR>Dates: $sd until $ed</font>
...
$sql = "SELECT `totals`.`id`, `totals`.`intime` , `totals`.`outtime` , `totals`.`no` , `totals`.`date1`, `totals`.`workedtime`  FROM `totals` WHERE `totals`.`date1`>='$dod' AND `totals`.`date1`<='$ddo' AND `totals`.`a1030`='0' AND `totals`.`workedtime`<='3'";
$q=$sql.$sortowanie;
$totalh=0;
if ($db->Query($q)) 
  {
