...
function WeekendsDays45($clockinginno,$BonusYearStart,$dod,$datedo){
	...
	$weekendaysfroman = "SELECT count(`no`)  as weekendyearcount  FROM `totals` WHERE `no`='$clockinginno' AND `totals`.`date1` >='$BonusYearStart'  AND `totals`.`date1` <='$datedo' AND (DATE_FORMAT(`date1`, \"%W\") = \"Saturday\" OR DATE_FORMAT(`date1`, \"%W\") = \"Sunday\" ) Group by no Order by date1 ASC;";
		if (!$dbWeekendsXX->Query($weekendaysfroman)) $dbWeekendsXX->Kill();
		$AllYearWeekendsDaysValue=$dbWeekendsXX->Rows();
...
if ($AllYearWeekendsDaysValue != $we45number) {
