...
$tytul='Would you like to delete this employee category date?';
...
<input name='id' type='hidden' value='$id'>
  ...
  <tr>
	...
	$sql = "SELECT `id`, `no`,DATE_FORMAT(`datechange`, \"%d/%m/%Y\") as datechange  FROM `emplcathistory` WHERE `id`='$id' LIMIT 1";
...
if (!$db->Open()) $db->Kill();
    ...
    $sql1 =("DELETE FROM `emplcathistory` WHERE `id`='$id' and `no`='$cln' LIMIT 1");

  if (!$db->Query($sql1)) $db->Kill();
  
  
  //$sql1 =("OPTIMIZE TABLE `emplcathistory` ");

  //if (!$db->Query($sql1)) $db->Kill();
  

echo "<script language='javascript'>window.location=\"emplcatlist.php?cln=$cln\"</script>";
} //if state=1
else
{
