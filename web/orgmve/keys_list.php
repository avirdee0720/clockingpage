...
FROM `nomberskeys`
LEFT JOIN nombers ON no = pno
LEFT JOIN keyslist ON `nomberskeys`.keysid = keyslist.id
LEFT JOIN `emplcat` ON `nombers`.`cat`= `emplcat`.catozn
WHERE `nomberskeys`.value <>0
AND  `nombers`.`pno` <> '5'
AND  `nombers`.`pno` <> '555'
AND  `emplcat`.catname_staff Like '$DepGroup'
";
...
$q = "
SELECT keysid,value,name,place FROM `nomberskeys` 
INNER JOIN keyslist ON `nomberskeys`.keysid = keyslist.id
where `no` = '$no' Order by keysid
...
$keystext="";
  ...
  while ($rq=$db->Row())
    {  
    if ($rq->value == '1') $keystext .= $rq->name.", ";
    }

	  
	   $keystext=substr($keystext, 0, -2);
  ...
  <td CLASS='$row_color' nowrap=\"nowrap\"><b><div align=\"right\">$keystext</div></b></td>
   </tr>";
