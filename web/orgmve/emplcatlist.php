...
if(!isset($sort)) {$sort="5";$kier_sql ="DESC";}
        ...
        switch ($sort)
                {
                case 1:
                 $sortowanie="  ORDER BY `emplcathistory`.`datechange` DESC, active ASC";
                 break;
                case 2:
                ...
                case 4:
                 ...
                 break;
                case 5:
                 $sortowanie=" ORDER BY `emplcathistory`.`datechange` $kier_sql";
                 break;
                default:
                 $sortowanie=" ORDER BY `emplcathistory`.`datechange`, active DESC  ";
                 break;
                }


if (!$db->Open()) $db->Kill();
$q = "SELECT `nombers`.`pno` , `nombers`.`surname` , `nombers`.`firstname` , `nombers`.`knownas` , `nombers`.`cat` ,`emplcat`.catname,DATE_FORMAT(`emplcathistory`.`datechange`, \"%d/%m/%Y\") As datechange,`emplcathistory`.`active`,`emplcathistory`.`id` FROM (`nombers` CROSS JOIN  `emplcathistory` ON `nombers`.`pno` = `emplcathistory`.`no`) INNER JOIN `emplcat` ON `emplcathistory`.`cat`= `emplcat`.catozn WHERE `nombers`.`pno` =  '$cln'";
$q=$q.$sortowanie;
...
$row_count=0;
  ...
  <td CLASS='$row_color' nowrap=\"nowrap\"><div align=\"right\"><input class='Input' size='10' maxlength='10' name='datechange_$row_count' value=\"$row->datechange\"></td>
	  <td CLASS='$row_color' nowrap=\"nowrap\"><div align=\"right\"><a href=\"del_emplcatlist.php?id=$row->id&cln=$cln\"><img src=\"images/delete_profile.gif\"></a></td>
...
<center>
<form action='$PHP_SELF' method='post' name='emplcatlist'>
...
<table border='0' cellpadding='2' cellspacing='1' CLASS='FormTABLE' width=\"98%\">
  <tr>
   ...
   <td CLASS='ColumnTD2' nowrap=\"nowrap\"><div align=\"center\">
                <A HREF='$PHP_SELF?sort=4&kier=$kier&cln=$cln'>Job title $kier_img[4]</a></td>
       </div>  </td>
       <td CLASS='ColumnTD2' nowrap=\"nowrap\"><div align=\"center\">
                Active?</td>
      </div>   </td>
...
elseif($state==1)
{
    ...
    for ($i = 0; $i < $row_count; $i++) {
        ...
        $sql ="UPDATE `emplcathistory` SET `emplcathistory`.datechange= STR_TO_DATE( '".$datechange[$i]."', '%d/%m/%Y') WHERE no='$cln' AND id='$id2[$i]]'
        LIMIT 1";
