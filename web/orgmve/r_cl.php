...
$db = new CMySQL;
$tytul='List of staff who has to clocking in the Office';
...
<table border='0' cellpadding='3' cellspacing='1' CLASS='FormTABLE'>
  <tr>
    ...
    <td CLASS='ColumnTD' nowrap>
            ...
            <td CLASS='ColumnTD' nowrap>
                <A HREF='$PHP_SELF?sort=5&kier=$kier'>In the office $kier_img[5]</a>
...
if(!isset($sort)) $sort=1;
        ...
        switch ($sort)
                ...
                case 4:
                 ...
                 break;
                case 5:
                 $sortowanie=" ORDER BY `nombers`.`cloffice` $kier_sql";
                 break;
                default:
                 $sortowanie=" ORDER BY `nombers`.`pno` $kier_sql ";
                 break;
...
if (!$db->Open()) $db->Kill();
$q = "SELECT `nombers`.`ID`, `nombers`.`pno`, `nombers`.`firstname`, `nombers`.`surname`, `nombers`.`knownas`, `nombers`.`status`, `nombers`.`cloffice`,`nombers`.`cloffice_only`, `emplcat`.`catname` FROM `nombers` RIGHT JOIN `emplcat` ON `nombers`.`cat`=`emplcat`.`catozn` WHERE `nombers`.`cloffice`='1' OR `nombers`.`cloffice_only`='1' ";
$q=$q.$sortowanie;
...
$row_count=0;
  ...
  if ($db->Query($q)) 
  {
	...
	echo "<tr>
		...
		<td CLASS='$row_color'>$row->cloffice</td>
	</tr>";
  } 
} else {
echo " 
