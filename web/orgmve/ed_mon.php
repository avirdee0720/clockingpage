...
if(!isset($state))
{
 ...
 if(isset($idmon)){
  $q = "SELECT id_mon, Nr_seryjny, MOdel, Nr_ewidencyjny, Producent, Rok_prod, id_zest FROM hd_monitor  WHERE id_mon='$idmon' LIMIT 1";
  } else { 
...
<tr>
        <td align='right' colspan='2'>
			...
			<input  name='idz' type='hidden' value='$idzest'>
...
if(isset($Nowy) and $Nowy=='$SAVEBTN')
	...
	if (!$db->Open())$db->Kill();
    $query =("UPDATE hd_monitor SET  Nr_ewidencyjny='$_Nr_ewid', Nr_seryjny='$_Nr_seryjny', 		  Rok_prod='$_Rok_prod', Producent='$_Dostawca', MOdel='$_Model' WHERE id_mon='$idmon' LIMIT 1 ");
	$result = mysql_query($query);
	$idkomp=mysql_insert_id();
	echo "<script language='javascript'>window.location=\"ed_mon.php?idmon=$idmon&idzest=$idz\"</script>";
	}
} 
?>