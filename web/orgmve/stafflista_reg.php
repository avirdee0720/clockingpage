...
if (!$db->Open()) $db->Kill();
$q = "SELECT `nombers`.`ID`, `nombers`.`pno`, `nombers`.`firstname`, `nombers`.`surname`, `nombers`.`knownas`, `nombers`.`status`, `emplcat`.`catname` FROM `nombers` RIGHT JOIN `emplcat` ON `nombers`.`cat`=`emplcat`.`catozn` WHERE `cat` <>'c' AND `cat` <>'ui' AND `cat` <>'ut' AND `status`='OK' ";
$q=$q.$sortowanie;
