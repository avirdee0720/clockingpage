...
if ($file == 'Cheque Details, 1008.csv') {

//$lines = implode(',',file($dir.$file));

$lines = file($dir.$file);

//echo "kllllll aaaaaaaaaaaaaaaaaaaaaa".$lines[0];
/*
...
VALUES ('".$data[$i][0]."','".$data[$i][1]."','".$data[$i][2]."','".$data[$i][3]."', STR_TO_DATE('".$data1d."', '%y-%m-%d'));
";
   // $sql = $db->Fix($sql);
    $query = $db->Query($sql);
...
$sql = " select 
 `bankdetails`.`pno`, `bankdetails`.`sortc`,`bankdetails`.`acno`,`bankdetails`.`bank`,`bankdetails`.`accname`,`bankdetails`.`forename`, `bankdetails`.`surename`,DATE_FORMAT(`bankdetails`.`date1` , \"%d/%m/%y\" ) AS date1,
`bankchequepayment`.`forename` As bankforename,`bankchequepayment`. `surename` As banksurename,`bankchequepayment`.`cheque`,`bankchequepayment`.`date1` as bankdate1
 from  `bankdetails` INNER JOIN `bankchequepayment`  on `bankdetails`.`pno`=`bankchequepayment`.`no`
Where bank='$bank'
 order by bank, surename
";
